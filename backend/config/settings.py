...
ALLOWED_HOSTS = [
    ...
    for host in config(
        "ALLOWED_HOSTS",
        default="dineops.up.railway.app,dineops-production.up.railway.app,localhost,127.0.0.1"
    ).split(",")
    if host.strip()
]
...
CSRF_TRUSTED_ORIGINS = [
    ...
    for origin in config(
        "CSRF_TRUSTED_ORIGINS",
        default="http://localhost:5173,http://127.0.0.1:5174,https://dine-ops.vercel.app"
    ).split(",")
    if origin.strip()
]
